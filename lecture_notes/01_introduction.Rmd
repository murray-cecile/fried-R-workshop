...
- there are a whole alternative set of common errors
...
- console vs. scripts
- Run commands by hitting enter (in the console) or highlighting and 
...
- Vectors: a list of values (numbers, strings, etc)
- Matrix: a group of vectors
- Data frame: a matrix, but with nice features 
- S4 object: a more complex kinds of user-defined data structure, we won't see them

In R, I most often work with vectors and dataframes. Vectors and matrices come up in many other programming languages for all kinds of purposes; data frames are also popular for data analysis.
...
The base R language is pretty powerful on its own. But the true power of R comes from user-built packages, which provide a bundle of new functionalities. 
...
Most data is available in a csv file format or something else that a spreadsheet program can open. Loading this into R is very simple using the `readr` package.

First, though, some key concepts:

- **working directory:** where on your computer R is looking to find files (scripts, data, etc.)
- **environment:** this is all the objects R is actively holding in memory for you
...
Crimes is a data frame, which is basically a more sophisticated kind of matrix.
...
Notice above that some of these names have backticks surrounding them. That's because they contain spaces, which are not a valid part of names in R. Fixing this and standardizing them will make working with the data much easier.
